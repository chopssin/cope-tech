...
Pages.use('/', params => {
  ...
  .res('save page', obj => {
    ...
    // Hydra >>>>>>>>>>>
    // This RES signal should be fired
    // as soon as we 
    //   1. click "<-" in SectionEditor, or 
    //   2. rearrange sections' orders, or
    //   3. change Page Title, URL Slug in page settings
    // 
    // obj should be an object of {
    //   key: 'sections' || 'pageTitle' || 'urlSlug'
    //   currPage: <current page id>
    //   data: <updates>
    // }
    //
  ...
  .res('save profile', obj => {
    ...
    // obj should be an object of {
    //   username: <string>,
    //   displayFile: <file object generated by Cope.openFiles>
    // }
    // <<<<<<<<<<<
  ...
  .res('save navigation', obj => {
    ...
    let currAppId = copeApp.get('currAppId');
    // Hydra >>>>>>>>>>>
    // arr should be an array of <string> pageIds
    // <<<<<<<<<<<

    // let G = Cope.graph(currAppId);
    // G.node('app').val('navigation', obj.navigation);
  })
  .res('new data', newData => {
    let currAppId = copeApp.get('currAppId');
    console.log(currAppId + ': new data', newData);
  });
